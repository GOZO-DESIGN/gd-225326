...
const sortedValues = (modules: Record<string, string>) =>
  Object.entries(modules).sort(([a], [b]) => a.localeCompare(b)).map(([, src]) => src);

const benjiImages = Object.entries(benjiModules)
  .sort(([a], [b]) => {
    const aB = a.includes("benji_"), bB = b.includes("benji_");
    if (aB && !bB) return -1;
    if (!aB && bB) return 1;
...
const galleries: Record<GalleryTab, { label: string; birthday: string; images: string[]; media?: MediaItem[] }> = {
  ...
  enzo: { label: "Enzo", birthday: "25.10.2025", images: sortedValues(enzoModules) },
  balu: { label: "Balu", birthday: "12.10.2025", images: sortedValues(baluModules) },
  panda: { label: "Panda", birthday: "12.10.2025", images: sortedValues(pandaModules) },
  "happy-hope": {
    label: "Happy Hope",
    birthday: "12.10.2025",
    images: happyHopeMedia.filter((m) => m.type === "image").map((m) => m.src),
    media: happyHopeMedia,
  },
};
      ...
      <section id="galerie" className="bg-primary-foreground py-16 md:py-20">
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            {Object.entries(galleries).map(([key]) => {
